'''
64 features in first conv layer
'''
...
def convolutional_neural_network(x):
    weights = {'W_conv1':tf.Variable(tf.random_normal([5,5,1,64])),
               ...
               'W_fc':tf.Variable(tf.random_normal([16*16*128,1024])),
    ...
    fc = tf.reshape(conv2,[-1, 16*16*128])
...
def train_neural_network(x):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            epoch_loss = 0
            for i in range(2):
                epoch_x = x_data[(i*90):(i*90)+90] 
                epoch_y = y_data[(i*90):(i*90)+90] 
